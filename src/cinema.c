...
void inicializar_compras(Compra* compras, int tam){
    ...
    compras[i].assento = 0;
    //o valor inicial √© 0, caso uma compra v· ser feita, È alterado para o preÁo do ingresso
    compras[i].valor = 0;
    compras[i].entrada = -1;
    ...
    strcpy(compras[i].categoria, "AÁ„o");
    }
...
void registrarcompra(Compra* compras, float valor_inteira, int cadeira_escolhida){
    //se o status da entrada È 0, È meia entrada, 1 È inteira, para assento, 0 È vazio e 1 È ocupado
...
int selecionar_assento(Compra* compras, int tam){
    ...
    if(compras[cadeira_escolhida].assento == 1){
        printf("Cadeira ocupada, deseja escolher outra? 1 para sim, 0 para n„o");
        scanf("%i", &continuar);
        if(continuar == 0){
            return -1;
            break;
        ...
        printf("O assento que voce escolheu nao existe! deseja escolher outro? 1 para sim, 0 para n„o\n: ");
        scanf("%i", &continuar);
        if(continuar == 0){
            return -1;
            break;}
...
FILE *arquivo = fopen(nome_arquivo, "r");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo! verifique se esse È realmente o nome do arquivo\n");
        return;
    }
char linha[200];
...
do {
    //Procura uma linha que come√ßa com um numero(Ou seja, a linha da matriz do mapa)
...
fclose(arquivo);
printf("Arquivo lido com sucesso! as informaÁıes foram atualizadas");
}
void imprimir_menu(){
printf("\n 0: sair\n 1: ver mapa dos ingressos\n 2: comprar assento\n 3: ver valor arrecadado\n 4: ver a quantidade de ingressos inteira e meia entrada adquiridos ate o momento\n 5: salvar como arquivo\n 6: Abrir e registrar arquivo previamente adquirido (isso atualizara a quantidade de ingressos e assentos adquiridos atÈ o momento)");
}
